...
if [ -z ${GNGT_BOARD_FIREBASE_DEPLOY_KEY} ]; then
  echo "Error: No access token for firebase specified." \
       "Please set the environment variable 'GNGT_DASHBOARD_ACCESS_TOKEN'."
  exit 1
fi
...
# will be deployed.
${firebaseBin} deploy --token ${GNGT_BOARD_FIREBASE_DEPLOY_KEY} \
  --non-interactive --project gngt-board
