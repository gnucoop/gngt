...
exports.runTypeScriptCompatibilityTest = async (tscBinPath) => {
  return new Promise((resolve, reject) => {
    const gngtDir = join(nodeModulesDir, '@gngt/');

    // Create the `node_modules/@gngt` directory in case it's not present.
    mkdir('-p', gngtDir);
...
/**
 * Gets all built Gngt NPM package artifacts by querying the Bazel runfiles.
...
function getNpmPackagesFromRunfiles() {
  ...
  const workspacePath = 'gc_gngt/src';
