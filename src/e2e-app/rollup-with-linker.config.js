...
const pouchdbResolutionPlugin = {
  resolveId: id => {
    const execRoot = process.cwd();
    const pouchdbDir = join(execRoot, 'node_modules/pouchdb');
    if (id === 'pouchdb') {
      return `${pouchdbDir}/dist/pouchdb.js`;
    }
    if (id === 'pouchdb-find') {
      return `${pouchdbDir}/dist/pouchdb.find.js`;
    }
    return null;
  },
}
