 ...
 */
export const apiDocsPackage = new Package('gngt-api-docs', [
  jsdocPackage,
  nunjucksPackage,
  typescriptPackage,
]);
...
apiDocsPackage.config((readTypeScriptModules: ReadTypeScriptModules, tsParser: TsParser) => {
  ...
  corePackages.forEach(packageName => {
    typescriptPathMap[`@gngt/core/${packageName}`] = [`./core/${packageName}/index.ts`];
  });

  materialPackages.forEach(packageName => {
    typescriptPathMap[`@gngt/material/${packageName}`] = [`./material/${packageName}/index.ts`];
  });

  ionicPackages.forEach(packageName => {
    typescriptPathMap[`@gngt/ionic/${packageName}`] = [`./ionic/${packageName}/index.ts`];
  });
...
apiDocsPackage.config((tsHost: Host) => {
  ...
  // various files in the Gngt project where the first node of a source file is exported and
