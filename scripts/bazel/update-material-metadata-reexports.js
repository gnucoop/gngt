...
/**
 * Script to manually add metadata re-exports to the @gngt/material package generated
 * via the ng_package bazel rule. This is rather special to the @gngt/material package,
 * so it hasn't been built into the ng_package rule yet.
 *
 * This is necessary for being able to import Angular constructs from "@gngt/material"
...
if (!fs.existsSync(materialPackageDir)) {
  console.error('The @gngt/material package has not been written to dist/bazel-packages.');
  process.exit(1);
}
...
// Read the root metadata for @gngt/material from the bazel output so we can modify it with
