...
export class PayloadChart {
  ...
  private createChartResults(data: PayloadResult[]) {
    ...
    data.forEach(result => {
      ...
      coreChartItems.push({ name: date, value: result.core_fesm_2015 });
    });

    return [
      { name: 'Material', series: materialChartItems },
      { name: 'Core', series: coreChartItems }
    ];
  }
