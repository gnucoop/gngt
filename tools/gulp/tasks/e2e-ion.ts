...
task(':watch:e2eapp-ion', () => {
  watchFiles(join(appDir, '**/*.ts'), ['e2e-app-ion:build'], false);
  watchFiles(join(appDir, '**/*.html'), ['e2e-app-ion:copy-assets'], false);
});
...
task(':serve:e2eapp-ion', serverTask(outDir, false, [
...
task(':serve:e2eapp-ion:stop', gulpConnect.serverClose);
