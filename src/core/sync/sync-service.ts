...
export class SyncService {
  ...
  get(tableName: string, params: ModelGetParams): Observable<any> {
    ...
    return this._databaseIsInit.pipe(
      ...
      exhaustMap(rmi => {
        const findPromise = db.find(
          this._modelGetFindRequest(
            tableName,
            params,
            rmi as PouchDB.Find.CreateIndexResponse<LocalDoc<any>>,
          ),
        );
        return from(findPromise).pipe(
          take(1),
          map(res => ({res, rmi})),
        );
      }),
          ...
          if (params.joins != null) {
            const observables = params.joins.map(join => {
              const findPromise = db.find(
                this._modelGetFindRequest(
                  join.model,
                  {id: obj[join.property], fields: join.fields},
                  rmi,
                ),
              );
              return from(findPromise).pipe(
  ...
  list(tableName: string, params: ModelListParams): Observable<ModelListResult<any>> {
    ...
    return this._databaseIsInit.pipe(
      ...
      exhaustMap(idx => {
        ...
        return from(db.find(findReq)).pipe(
          take(1),
          catchError(err => {
          ...
          return zip(
            ...params.joins.map(join => {
              ...
              return from(db.find(req)).pipe(
                ...
                map(related => ({
                  join,
                  related: (related as PouchDB.Find.FindResponse<LocalDoc<any>>).docs,
                })),
              );
            }),
          ).pipe(
  ...
  create(tableName: string, object: any): Observable<any> {
    return this._databaseIsInit.pipe(
      ...
      exhaustMap(id => {
        ...
        return from(this._database.post({...localDoc, object} as any)).pipe(
          exhaustMap(d => {
            const doc = d as PouchDB.Core.Response;
            return this._createLocalSyncEntry({
              doc_id: doc.id,
              entry_type: 'insert',
              ...localDoc,
            } as Partial<LocalSyncEntry>).pipe(map(() => object));
          }),
        ) as Observable<any>;
      }),
      take(1),
    ...
    return this._databaseIsInit.pipe(
      ...
      exhaustMap(r => {
        const {res, localDoc} = r as {
          ...
          localDoc: PouchDB.Core.ExistingDocument<LocalDoc<any>>;
        };
  ...
  deleteAll(tableName: string, ids: number[]): Observable<any[]> {
    ...
    return this._databaseIsInit.pipe(
      ...
      concatMap(r => {
        ...
        if (res.docs.length !== 1) {
          return throwError('not_found') as Observable<
            PouchDB.Core.ExistingDocument<LocalDoc<any>>
          >;
        }
      ...
      concatMap(ld => {
        ...
        return from(db.remove(localDoc)).pipe(map(res => ({res, localDoc})));
      }),
      concatMap(r => {
        const {res, localDoc} = r as {
          res: PouchDB.Core.Response;
  ...
  query(tableName: string, params: ModelQueryParams): Observable<ModelListResult<any>> {
    ...
    return this._databaseIsInit.pipe(
      ...
      exhaustMap(idx => {
        ...
        return (
          from(db.find(findReq)) as Observable<PouchDB.Find.FindResponse<LocalDoc<any>>>
        ).pipe(
          take(1),
          catchError(err => {
            if (err.error === 'no_usable_index') {
      ...
      switchMap(r => {
        const res = r as PouchDB.Find.FindResponse<LocalDoc<any>>;
        return (
          this._relationalModelIdxObs() as Observable<
            PouchDB.Find.CreateIndexResponse<LocalDoc<any>>
          >
  ...
  private _setLocalSyncNumber(syncNumber: number): Observable<number> {
    ...
    return from(db.get(id)).pipe(
      ...
      exhaustMap(doc =>
        from(db.post({...(doc as LocalSyncNumber), number: syncNumber})).pipe(take(1)),
      ),
      map(_ => syncNumber),
    );
  }
  ...
  private _getNextLocalSyncNumber(): Observable<number> {
    const db = this._getLocalSyncNumberDb();
    return from(db.get(`_local/${this._localSyncNumber}`)).pipe(
      take(1),
  ...
  private _nextObjectId(tableName: string): Observable<number> {
    ...
    return this._relationalModelIdxObs({tableName, sort}).pipe(
      exhaustMap(idx =>
        (
          from(db.find(this._modelListFindRequest(tableName, {sort}, idx))) as Observable<
  ...
  private _relationalModelIdxObs(idxDef?: {
    tableName: string;
    ...
    return (
      from(this._database.createIndex(this._relationalModelIdx)) as Observable<
        PouchDB.Find.CreateIndexResponse<LocalDoc<any>>
      >
    ).pipe(take(1));
  }
      ...
      .pipe(
        exhaustMap(r => {
          ...
          for (let i = firstId; i <= lastId; i++) {
            gets.push(
              from(
                localSyncDb.get(`_local/${this._localSyncEntryPrefix}${i}`),
              ) as Observable<LocalSyncEntry>,
            );
          }
          return zip(...gets).pipe(
            exhaustMap(getsRes => {
              ...
              return from(localDocsDb.allDocs(opts)).pipe(
                map(ld => {
                  ...
                  const docs = entries.map((syncEntry, i) => ({
                    syncEntry,
                    localDoc: localDocs.rows[i].doc!,
                  }));
                  return {hasNext, docs};
                }),
                take(1),
            ...
            exhaustMap(res =>
              this._processUpwardChanges(res as {hasNext: boolean; docs: UpwardChange[]}),
            ),
            take(1),
          );
        }),
    ...
    return this._httpClient.post<UpwardChangeResult[]>(this._syncUrl, payload).pipe(
      ...
      exhaustMap(r => {
        ...
        return from(
          ...
          exhaustMap(result =>
            from(
              localDocsDb.bulkDocs(
                (result as PouchDB.Core.AllDocsResponse<LocalDoc<any>>).rows.map(
      ...
      exhaustMap(sequence =>
        (sequence as number) >= 0 ? this._setLastLocalCheckpoint(sequence as number) : obsOf(null),
      ),
      map(_ => p.hasNext),
    );
  }
    ...
    this._httpClient
      ...
      .pipe(
        ...
        switchMap(docs => {
          ...
          ) as Observable<{change: number; doc: SyncEntry}>;
        }),
  ...
  private _modelBulkIdsFindRequest(
    ...
    const req = {
      selector: {table_name: tableName, object_id: {$in: ids}},
    } as PouchDB.Find.FindRequest<LocalDoc<any>>;
    if (index != null) {
      const idxParts = ((index as any).id || '').split('/');
      if (idxParts.length === 2) {
        req.use_index = idxParts[1];
  ...
  private _syncEntryFindRequest(
    ...
    const req = {selector: this._syncEntryFindSelector(entry)} as PouchDB.Find.FindRequest<
      LocalDoc<any>
    >;
    if (index != null) {
      const idxParts = ((index as any).id || '').split('/');
  ...
  private _initLocalDatabase(): void {
    ...
    this._database
      .createIndex(this._relationalModelIdx)
      .then(_ => {
        this._emitSyncPaused();
